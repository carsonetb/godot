...
void SteamIntegration::_bind_methods() {
    ClassDB::bind_method(D_METHOD("_on_lobby_created", "connect", "this_lobby_id"), &SteamIntegration::_on_lobby_created);
    ClassDB::bind_method(D_METHOD("_on_lobby_match_list", "these_lobbies"), &SteamIntegration::_on_lobby_match_list);
    ClassDB::bind_method(D_METHOD("_on_lobby_joined", "this_lobby_id", "_permissions", "_locked", "response"), &SteamIntegration::_on_lobby_joined);
    ClassDB::bind_method(D_METHOD("_on_lobby_chat_update", "this_lobby_id", "change_id", "making_change_id", "chat_state"), &SteamIntegration::_on_lobby_chat_update);
    ClassDB::bind_method(D_METHOD("_on_p2p_session_request", "remote_id"), &SteamIntegration::_on_p2p_session_request);
    ClassDB::bind_method(D_METHOD("_on_p2p_session_connect_fail", "steam_id", "session_error"), &SteamIntegration::_on_p2p_session_connect_fail);
    
    ADD_SIGNAL(MethodInfo("handshake_completed", PropertyInfo(Variant::INT, "with")));
}

void SteamIntegration::_notification(int what) {
...
void SteamIntegration::_ready() {
    ...
    if ((int)initialize_response["status"] > STEAM_API_INIT_RESULT_OK) {
        ...
        print_line("Using backup non-steam mode.");
        platform = "backup";
        steam_id = 0;
        steam_name = "You";
        return;
...
void SteamIntegration::_process() {
    if (lobby_id > 0) {
        _read_all_p2p_packets(0);
    }
}

void SteamIntegration::_create_lobby() {
    if (lobby_id == 0) {
        is_lobby_owner = true;
...
void SteamIntegration::_join_lobby(uint64_t this_lobby_id) {
    if (MultiGodot::VERBOSE_DEBUG) {
...
void SteamIntegration::_get_lobby_members() {
    lobby_members.clear();
...
void SteamIntegration::_make_p2p_handshake() {
    if (MultiGodot::VERBOSE_DEBUG) {
...
void SteamIntegration::_send_p2p_packet(uint64_t this_target, Dictionary packet_data, P2PSend custom_send_type, int custom_channel) {
...
void SteamIntegration::_send_p2p_packet(uint64_t this_target, Dictionary packet_data) {
    _send_p2p_packet(this_target, packet_data, SEND_TYPE, DEFAULT_CHANNEL);
}

void SteamIntegration::_read_all_p2p_packets(int read_count) {
...
void SteamIntegration::_read_p2p_packet() {
    ...
    if (packet_size > 0) {
        ...
        if (!handshake_completed_with.has(packet_sender)) {
            ...
            emit_signal("handshake_completed", packet_sender);
...
void SteamIntegration::_leave_lobby() {
    if (lobby_id != 0) {
...
void SteamIntegration::_on_lobby_created(int connect, uint64_t this_lobby_id) {
    if (connect == 1) {
...
void SteamIntegration::_on_lobby_match_list(Array these_lobbies) {
    if (MultiGodot::VERBOSE_DEBUG) {
...
void SteamIntegration::_on_lobby_joined(uint64_t this_lobby_id, int _permissions, bool _locked, int response) {
...
void SteamIntegration::_on_lobby_chat_update(uint64_t this_lobby_id, uint64_t change_id, uint64_t making_change_id, int chat_state) {
...
void SteamIntegration::_on_p2p_session_request(uint64_t remote_id) {
...
void SteamIntegration::_on_p2p_session_connect_fail(uint64_t this_steam_id, int session_error) {
...
void SteamIntegration::sync_var(Node *node, StringName property, uint64_t custom_target) {
...
void SteamIntegration::call_func(Node *node, String function_name, Array args, uint64_t custom_target) {
    NodePath path = editor_node_singleton->get_path_to(node);
    auto send_dictionary = Dictionary({
